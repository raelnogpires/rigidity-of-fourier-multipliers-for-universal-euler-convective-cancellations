...
a dense collection of triads. No result on PDE existence, smoothness, or
regularity is claimed anywhere in this paper.
...
The new results are the all-symbol implication (Theorem~A), the automatic
Hermiticity in the solenoidal class (Corollary~B), and the twelve-dimensional
unrestricted longitudinal kernel; the standard facts recalled above ---
conservation of energy and helicity, their triad manifestations, the helical
decomposition, the local conservation-law and Casimir classifications, the
triad-network nullspace methodology, and the scalar Laplacian
strain--vorticity identity --- are not claimed as new.

\subsection{Outline}
\label{subsec:outline}

Sections~\ref{sec:setting}--\ref{sec:results} fix conventions and state the
results. Sections~\ref{sec:sufficiency}--\ref{sec:propagation} prove
sufficiency, the triad lemmas, the finite seeds and the lattice propagation.
Section~\ref{sec:singular} gives the singular spectra, Section~\ref{sec:strainvorticity}
the strain--vorticity applications, and Section~\ref{sec:computer} the
computer-assisted components and formalization. Section~\ref{sec:discussion}
lists open problems.
...
First recover the shell-2 axis modes $2e_{i}$ by the two decompositions
\eqref{eq:axisraw} at $n=2$, whose four parents $e_{i}\pm e_{j}$ and
$e_{i}\pm e_{\ell}$ all lie in $\Lambda_{1}$; every non-axis mode in shells
$1$ and $2$ already lies in the coordinate unit cube. Then induct on
$|t|_{1}\ge3$. Within each shell, process all non-axis targets before the
axis targets.
...
\eqref{eq:axisraw} recover each axis, starting from the shell-2 axes
$2e_{i}$, whose parents all lie in $\Lambda_{1}$. Thus no target is omitted by either
ordering.
...
the zero Fourier block is invisible rather than rigid. No classification of
nonlinear, time-dependent, or non-translation-invariant functionals is
attempted; the multiplier class considered throughout is translation-invariant
by construction.
...
\href{https://doi.org/10.5281/zenodo.22819121}{\texttt{10.5281/zenodo.22819121}},
which resolves to the most recent version; the materials corresponding to this
version are release
\href{https://github.com/raelnogpires/rigidity-of-fourier-multipliers-for-universal-euler-convective-cancellations/releases/tag/v1.0.4}{\texttt{v1.0.4}},
whose own version DOI is listed on that record. The archived materials are
